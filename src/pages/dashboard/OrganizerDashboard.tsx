    ...
    >
      {isLoading ? (
        ...
        <>
          ...
          {view === "announcements" && <AnnouncementsView organizerId={organizer.id} />}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem
                          onClick={() => {
                            const url = prompt("Floor plan URL:", e.floor_plan_url ?? "");
                            if (url !== null) update.mutate({ id: e.id, patch: { floor_plan_url: url || null } });
                          }}
                        >
...
/* ---------------- Announcements (UC-O02) ---------------- */
function AnnouncementsView({ organizerId }: { organizerId: string }) {
  const qc = useQueryClient();
  const { data: events = [] } = useQuery({
    queryKey: ["events", organizerId],
    queryFn: () => listEvents(organizerId),
  ...
  const create = useMutation({
    mutationFn: () => createAnnouncement({ eventId, title, body: body || undefined }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["announcements", eventId] });
      toast.success("โพสต์ announcement แล้ว");
      setTitle(""); setBody("");
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  if (events.length === 0) {
    return (
      <Card className="p-12 text-center">
        ...
        <p className="text-sm text-muted-foreground">ยังไม่มี event — สร้าง event ก่อนถึงโพสต์ announcement ได้</p>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      <Card className="p-5 glass">
        <div className="flex items-center gap-3 mb-3">
          <h2 className="font-semibold">โพสต์ Announcement</h2>
          <Select value={eventId} onValueChange={setEventId}>
        ...
        <div className="space-y-3">
          <div>
            <Label>หัวข้อ</Label>
            <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="เช่น พิธีเปิดเลื่อนเป็น 10:30" />
          </div>
          <div>
            <Label>เนื้อความ (optional)</Label>
            <Textarea rows={3} value={body} onChange={(e) => setBody(e.target.value)} />
          </div>
          <Button
            className="bg-gradient-primary"
            onClick={() => create.mutate()}
            disabled={!eventId || !title || create.isPending}
          >
            {create.isPending && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
            <Plus className="h-4 w-4 mr-1" /> โพสต์
          </Button>
        </div>
      </Card>
      ...
      <Card className="glass">
        <div className="p-4 border-b flex items-center justify-between">
          <h3 className="font-semibold text-sm">Announcements ก่อนหน้า</h3>
          <Badge variant="outline" className="font-mono text-xs">{announcements.length}</Badge>
        </div>
        <div className="divide-y">
          {isLoading && <div className="p-6 grid place-items-center"><Loader2 className="h-5 w-5 animate-spin" /></div>}
          {!isLoading && announcements.length === 0 && (
            <div className="p-8 text-center text-sm text-muted-foreground">ยังไม่มี announcement</div>
          )}
          {announcements.map((a) => (
            <div key={a.id} className="p-4 flex items-start justify-between gap-3 hover:bg-muted/30 transition-colors">
              <div className="min-w-0">
              ...
              <Button
                ...
                onClick={() => { if (confirm("ลบ announcement นี้?")) del.mutate(a.id); }}
              >
